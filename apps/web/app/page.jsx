...
export default function Home() {
  ...
  return (
    <main className="site-shell">
      <div className="storage-strip" role="status" aria-label="FITFLOW local storage status"><span>LOCAL-FIRST STORAGE</span><b>{savedProfile ? 'PROFILE SAVED' : 'PROFILE READY'}</b><b>{savedPlan ? 'PLAN SAVED' : 'PLAN NOT SAVED'}</b><b>{workoutHistory.length} WORKOUT{workoutHistory.length === 1 ? '' : 'S'} LOGGED</b><a className="storage-dashboard-link" href="/dashboard">OPEN DASHBOARD →</a></div>
